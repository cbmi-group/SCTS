...
class FCNMaskHead(nn.Module):
    ...
    def get_targets(self, sampling_results, gt_masks, rcnn_train_cfg):
        ...
        if 'return_mask_all' in rcnn_train_cfg and rcnn_train_cfg['return_mask_all']==True:
            mask_targets,mask_all = mask_target(pos_proposals, pos_assigned_gt_inds,
                                    gt_masks, rcnn_train_cfg, rcnn_train_cfg['return_mask_all'])
            # predict_semantics = []
            # import ipdb
            # ipdb.set_trace()
            # for i in range(len(mask_all)):
            #     predict_semantic = np.zeros(mask_all[i].shape)
            #     predict_sum = np.zeros(mask_all[i].shape)
            #     for pos_pro in pos_proposals[i]:
            #         predict_semantic[int(pos_proposals[0][0][1]):int(pos_proposals[0][0][3]),int(pos_proposals[0][0][0]):int(pos_proposals[0][0][2])] = mask_all[i][int(pos_proposals[0][0][1]):int(pos_proposals[0][0][3]),int(pos_proposals[0][0][0]):int(pos_proposals[0][0][2])].shape
                    

            return mask_targets,mask_all
        else:
