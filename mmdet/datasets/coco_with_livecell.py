...
class CocoDataset(CustomDataset):
    ...
    def post_process(self,result_files):
        iou_thr = 0.3
        ioa_thr = 0.8
        del_indexs = []
        segm_json_file = result_files['segm']
        bbox_json_file = result_files['bbox']
        all_segm = defaultdict(list)
        
        with open(segm_json_file,"r") as f:
            load_dict = json.load(f)
            new_load_dict = []
            print("before_dict",len(load_dict))
            scores = []

            ### get_maskx
            for i in range(len(load_dict)):
                scores.append(load_dict[i]['score'])
                segm = load_dict[i]["segmentation"]
                mask = mask_util.decode(segm)[:, :]
                ### updata_mask 
                # The results of multiple Unicom channels are transferred to one
                num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(mask, connectivity=8)
                if num_labels > 2:
                    mask = np.array(labels == np.argmax(stats[1:,-1]) + 1).astype(np.uint8)  
                all_segm[load_dict[i]['image_id']].append(mask)

            ### cal_iou
            for image_id in all_segm.keys():
                segm_this_image = all_segm[image_id]
                ...
                for i  in range(len(segm_this_image)):
                    for j in range(len(segm_this_image)):
                        if j > i:
                            iou_map[i][j],ioa_map[i][j] = self.cal_iou(segm_this_image[i],segm_this_image[j])
                ...
                if len(iou_res[0]) > 0:
                    ...
                    for i in range(len(iou_res[0])):
                        ...
                        del_indexs.append(min_index) 
            
            print(len(del_indexs))
            for i in range(len(load_dict)):
                if i not in del_indexs:
                    new_load_dict.append(load_dict[i])
            
            print("after_dict",len(load_dict))
            
            with open(segm_json_file,"w") as out:
                json.dump(new_load_dict,out)
        
        # return load_dict
    
    def vis_res(self,result_files):
        segm_json_file = result_files['segm']
        bbox_json_file = result_files['bbox']
        with open(segm_json_file,"r") as f:
            load_dict = json.load(f)
    ...
    #    print("_evaluate_predictions_on_coco")
        ...
        return coco_eval,np.round(np.array(pre_per_iou), 4),np.round(np.array(rec_pre_iou), 4)
    ...
    def evaluate(self,
        ...
        #### outputs_to_mask
        # self.post_process(result_files)
        
        ## def vis_res(result_files)
        # import ipdb
        # ipdb.set_trace()
        ...
        for metric in metrics:
            ...
            try:
                coco_temp = json.load(open(result_files[metric],"r"))
                
                cocoDt = cocoGt.loadRes(result_files[metric])
                # print(cocoDt[0])
                eval_out,pre_out,rec_out = self._evaluate_predictions_on_coco(cocoGt,coco_temp,iou_type=metric,use_fast_impl=_use_fast_impl)
                metric_items = [
                        'AP-mAP', 'AP-mAP_50', 'AP-mAP_75', 'AP-mAP_s', 'AP-mAP_m', 'AP-mAP_l','AR-mAP', 'AR-mAP_50', 'AR-mAP_75', 'AR-mAP_s', 'AR-mAP_m', 'AR-mAP_l'
                    ]
                metric_items = [item_name+"_"+str(metric) for item_name in metric_items]
                # [0.5 , 0.55, 0.6 , 0.65, 0.7 , 0.75, 0.8 , 0.85, 0.9 , 0.95]
                metrci_AP = ['mAP0.5', 'mAP0.55','mAP0.6','mAP0.65','mAP0.7','mAP0.75','mAP0.8', 'mAP0.85','mAP0.9','mAP0.95']
                metrci_AR = ['mAR0.5', 'mAR0.55','mAR0.6','mAR0.65','mAR0.7','mAR0.75','mAR0.8', 'mAR0.85','mAR0.9','mAR0.95']
                metrci_AP = [item_name+"_"+str(metric) for item_name in metrci_AP]
                metrci_AR = [item_name+"_"+str(metric) for item_name in metrci_AR]
                for i in range(len(eval_out.stats)):
                    eval_results[metric_items[i]] = eval_out.stats[i]
                for i in range(len(pre_out)):
                    eval_results[metrci_AP[i]] = pre_out[i]
                for i in range(len(rec_out)):
                    eval_results[metrci_AR[i]] = rec_out[i]
                
                # self._evaluate_predictions_on_coco(cocoGt,cocoDt,iou_type=metric,use_fast_impl=_use_fast_impl)
                ### eval todo
            except IndexError:
                print_log(
        ...
        #     cocoEval = (COCOeval_opt if _use_fast_impl else COCOeval)(cocoGt, cocoDt, iou_type)
