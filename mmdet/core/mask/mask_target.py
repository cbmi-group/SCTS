...
def mask_target(pos_proposals_list, pos_assigned_gt_inds_list, gt_masks_list,
    ...
    if return_mask_all:
        mask_targets = []
        mask_all = []
        temp= list(map(mask_target_single, pos_proposals_list,pos_assigned_gt_inds_list, gt_masks_list, cfg_list, [return_mask_all]*len(pos_proposals_list)))
        for i in range(len(temp)):
            mask_targets.append(temp[i][0])
            mask_all.append(temp[i][1])
    else:
...
def mask_target_single(pos_proposals, pos_assigned_gt_inds, gt_masks, cfg, return_mask_all=False):
    ...
    if num_pos > 0:
        ...
        if return_mask_all:
            temp = gt_masks.crop_and_resize(
                proposals_np, mask_size, device=device,
                inds=pos_assigned_gt_inds,return_mask_all=return_mask_all)
            mask_targets = temp[0].to_ndarray()
            mask_all = temp[1]
        else:
        ...
        if return_mask_all:
            mask_all = gt_masks.crop_and_resize(
                proposals_np, mask_size, device=device,
                inds=pos_assigned_gt_inds).to_ndarray()
